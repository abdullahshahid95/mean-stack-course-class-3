function getOrder() {
    ...
    var loadChineseMenu;
    var loadRefreshmentMenu;
    var duration;
    var tableNumber;
    //================Pakistani Menu===========================//
    var loadPakMenu = parseInt(prompt("Load Pakistani Menu?\n1 for yes, 0 for no."));
    if (loadPakMenu == 1) {
        var pakMenuChoice = prompt("Enter the number related to the choice."
        ...
        // + "10. Naan Rs 20");
        if (pakMenuChoice != "0") {
            var choiceArray1 = pakMenuChoice.split(",");
            choiceArray1.forEach(function (element) {
    ...
    if (loadChineseMenu == 1) {
        var chineseMenuChoice = prompt("Enter the number related to the choice."
            + "Separate with comma for multiple choices.\n\n"
            + "0. For no item\n"
        ...
        if (chineseMenuChoice != "0") {
            var choiceArray2 = chineseMenuChoice.split(",");
            choiceArray2.forEach(function (element) {
    ...
    if (loadRefreshmentMenu == 1) {
        var refreshmentMenuChoice = prompt("Enter the number related to the choice."
            + "Separate with comma for multiple choices.\n\n"
            + "0. For no item\n"
            + "1. Tea Rs 20\n"
            + "2. Mineral Water Rs 30\n");
        if (refreshmentMenuChoice != "0") {
            var choiceArray3 = refreshmentMenuChoice.split(",");
            choiceArray3.forEach(function (element) {
    ...
    if (pakOrder.length > 0) {
        result += "\nPakistani Menu";
        pakOrder.forEach(function (element) {
            total += element.price;
    ...
    if (chineseOrder.length > 0) {
        result += "\n";
        result += "\nChinese Menu";
        chineseOrder.forEach(function (element) {
            total += element.price;
            result += "\n" + element.name + ":Rs " + element.price + " x " + element.amount
                + " Rs " + (element.price * element.amount);
        });
    ...
    if (refreshmentOrder.length > 0) {
        result += "\n";
        result += "\nRefreshment Menu";
        refreshmentOrder.forEach(function (element) {
            total += element.price;
            result += "\n" + element.name + ":Rs " + element.price + " x " + element.amount
                + " Rs " + (element.price * element.amount);
        });
